...
def test_RiverRun_set_contact_need_yield_SetsAttr(temp3_dir):
    ...
    # WHEN
    yao_need_yield = 7
    bob_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
...
def test_RiverRun_need_yields_is_empty_ReturnsObj(temp3_dir):
    ...
    # WHEN
    yao_need_yield = 500
    x_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
    # THEN
    ...
    # WHEN
    bob_need_yield = 300
    x_riverrun.set_contact_need_yield(exx.yao, bob_need_yield)
    x_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
    # THEN
    assert x_riverrun.need_yields_is_empty() is False
...
def test_RiverRun_reset_need_yields_SetsAttr(temp3_dir):
    ...
    sue_need_yield = 56
    yao_need_yield = 6
    bob_riverrun.set_contact_need_yield(exx.bob, bob_need_yield)
    bob_riverrun.set_contact_need_yield(exx.sue, sue_need_yield)
    bob_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
...
def test_RiverRun_contact_has_need_yield_ReturnsBool(temp3_dir):
    # ESTABLISH
    mstr_dir = temp3_dir
    bob_mana_amount = 1000
    bob_mana_grain = 1
    ...
    bob_riverrun.set_contact_need_yield(exx.sue, sue_need_yield)
    bob_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
    assert bob_riverrun.contact_has_need_yield(exx.bob)
    assert bob_riverrun.contact_has_need_yield(exx.sue)
    assert bob_riverrun.contact_has_need_yield(exx.yao)
    assert bob_riverrun.contact_has_need_yield(exx.zia) is False

    # WHEN
    bob_riverrun.reset_need_yields()

    # THEN
    assert bob_riverrun.contact_has_need_yield(exx.bob) is False
    assert bob_riverrun.contact_has_need_yield(exx.sue) is False
    assert bob_riverrun.contact_has_need_yield(exx.yao) is False
    assert bob_riverrun.contact_has_need_yield(exx.zia) is False
...
def test_RiverRun_delete_need_yield_SetsAttr(temp3_dir):
    ...
    bob_riverrun.set_contact_need_yield(exx.yao, 5)
    assert bob_riverrun.contact_has_need_yield(exx.yao)
...
def test_RiverRun_get_contact_need_yield_ReturnsObj(temp3_dir):
    # ESTABLISH
    mstr_dir = temp3_dir
    bob_mana_amount = 1000
    bob_mana_grain = 1
    ...
    bob_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
    assert bob_riverrun.contact_has_need_yield(exx.bob)
    assert bob_riverrun.get_contact_need_yield(exx.bob) == bob_need_yield
    assert bob_riverrun.contact_has_need_yield(exx.zia) is False
    assert bob_riverrun.get_contact_need_yield(exx.zia) == 0
    ...
    # THEN
    assert bob_riverrun.contact_has_need_yield(exx.bob) is False
    assert bob_riverrun.get_contact_need_yield(exx.bob) == 0
    assert bob_riverrun.contact_has_need_yield(exx.zia) is False
    assert bob_riverrun.get_contact_need_yield(exx.zia) == 0


def test_RiverRun_add_contact_need_yield_ReturnsObj(temp3_dir):
    # ESTABLISH
    mstr_dir = temp3_dir
    bob_mana_amount = 1000
    bob_mana_grain = 1
    ...
    bob_riverrun.set_contact_need_yield(exx.yao, yao_need_yield)
    assert bob_riverrun.get_contact_need_yield(exx.bob) == bob_need_yield
    assert bob_riverrun.get_contact_need_yield(exx.sue) == sue_need_yield
    assert bob_riverrun.get_contact_need_yield(exx.zia) == 0

    # WHEN
    bob_riverrun.add_contact_need_yield(exx.sue, 5)
    bob_riverrun.add_contact_need_yield(exx.zia, 10)

    # THEN
    assert bob_riverrun.get_contact_need_yield(exx.bob) == bob_need_yield
    assert bob_riverrun.get_contact_need_yield(exx.sue) == sue_need_yield + 5
    assert bob_riverrun.get_contact_need_yield(exx.zia) == 10
...
def test_RiverRun_levy_need_due_SetsAttr_ScenarioY(temp3_dir):
    ...
    bob_person.add_contactunit(exx.bob, 2, bob_need_yield)
    bob_person.add_contactunit(exx.sue, 2, sue_need_yield)
    bob_person.add_contactunit(exx.yao, 2, yao_need_yield)
    ...
    assert bob_riverrun.get_contact_need_yield(exx.bob) == 5
    ...
    assert bob_riverrun.get_contact_need_yield(exx.bob) == 380

    # ESTABLISH
    assert bob_riverrun.get_contact_need_due(exx.sue) == 560
    assert bob_riverrun.get_contact_need_yield(exx.sue) == 0
    # WHEN
    ...
    assert bob_riverrun.get_contact_need_yield(exx.sue) == 560
    ...
    assert bob_riverrun.get_contact_need_yield(exx.yao) == 0
    # WHEN
    ...
    assert bob_riverrun.get_contact_need_yield(exx.yao) == 60
