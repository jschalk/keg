...
def get_moment_guts_contacts_dataframe(x_moment: MomentUnit) -> DataFrame:
    ...
    for person_name in moment_person_names:
        ...
        gut_person.conpute()
        df = get_person_contactunits_dataframe(gut_person)
        df.insert(0, "person_name", gut_person.person_name)
        gut_dfs.append(df)
    return pandas_concat(gut_dfs, ignore_index=True)


def get_moment_guts_contacts_plotly_fig(x_moment: MomentUnit) -> plotly_Figure:
    column_header_list = [
        "person_name",
        "contact_name",
        "contact_cred_lumen",
    ...
    df = get_moment_guts_contacts_dataframe(x_moment)
    ...
    fig_label = f"moment '{x_moment.moment_rope}', gut contacts metrics"
...
def get_moment_jobs_contacts_dataframe(x_moment: MomentUnit) -> DataFrame:
    # get list of all person paths
    moment_person_names = x_moment._get_person_dir_names()
    # for all persons get gut
    job_dfs = []
    for person_name in moment_person_names:
        ...
        job.conpute()
        job_df = get_person_contactunits_dataframe(job)
        job_df.insert(0, "person_name", job.person_name)
        job_dfs.append(job_df)
    return pandas_concat(job_dfs, ignore_index=True)


def get_moment_jobs_contacts_plotly_fig(x_moment: MomentUnit) -> plotly_Figure:
    column_header_list = [
        "person_name",
        "contact_name",
        "contact_cred_lumen",
    ...
    df = get_moment_jobs_contacts_dataframe(x_moment)
    header_dict = dict(values=column_header_list, fill_color="powderblue", align="left")
    x_table = plotly_Table(
        header=header_dict,
        cells=dict(
    ...
    fig = plotly_Figure(data=[x_table])
    fig_label = f"moment '{x_moment.moment_rope}', job contacts metrics"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_label, title_font_size=20)
    return fig
