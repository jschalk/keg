...
def test_create_brick_table_from_csv_ChangesDBState(
    ...
    get_brick_sqlite_types_columns = [
        (0, kw.spark_num, column_types.get(kw.spark_num), 0, None, 0),
        (1, kw.spark_face, column_types.get(kw.spark_face), 0, None, 0),
    ...
    assert columns == get_brick_sqlite_types_columns


def test_insert_brick_csv_ChangesDBState_add_to_empty_table(
    setup_database_and_csv: tuple[sqlite3_Connection, str, str],
):
    # ESTABLISH
    conn, test_csv_filepath = setup_database_and_csv
    bk_tablename = "bkXXXXX"
    create_brick_table_from_csv(test_csv_filepath, conn, bk_tablename)
    cursor = conn.cursor()
    cursor.execute(f"SELECT * FROM {bk_tablename}")
    assert cursor.fetchall() == []

    # WHEN Call the function to insert data from the CSV file into the database
    insert_brick_csv(test_csv_filepath, conn, bk_tablename)
...
def test_insert_brick_csv_ChangesDBState_Inserts(
    ...
    zia_csv_filepath = "zia_bkXXXXX.csv"
    ...
    # WHEN
    insert_brick_csv(zia_csv_filepath, conn, bk_tablename)
...
def test_insert_brick_csv_ChangesDBState_CanCreateTable(
    setup_database_and_csv: tuple[sqlite3_Connection, str, str],
):
    """Test the insert_csv function using pytest."""
    # ESTABLISH
    ...
    bk_tablename = "bkXXXXX"
    cursor = conn.cursor()
    cursor.execute(f"PRAGMA table_info({bk_tablename})")
    columns = cursor.fetchall()
    assert columns == []
