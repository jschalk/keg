...
def test_get_brick_format_filenames_ReturnsObj_Validate_brick_format_files():
    ...
    for brick_filename in brick_filenames_sorted:
        ...
        for brick_format_dimen in format_dimens:
            assert brick_format_dimen in valid_idea_dimens

        assert ref_dict.get(kw.attributes)
        brick_format_attributes = ref_dict.get(kw.attributes)
        for idea_attribute, attr_dict in brick_format_attributes.items():
            ...
            for brick_format_dimen in format_dimens:
                ...
                assert_fail_str = (
                    f"{brick_format_dimen=} {idea_attribute=} {otx_key_value=}"
                )
        ...
        for x_dimen, dimen_keys in all_dimen_keys_dict.items():
            ...
            #     print(f"{brick_type_value}  {x_dimen=} {idea_attrs_list=}")
            if dimen_keys.issubset(idea_attrs):
                if x_dimen not in format_dimens:
                    print(f"MISSING {x_dimen=} {brick_type_value} {idea_attrs=}")
                ...
                if x_dimen in format_dimens:
                    print(
                        f"SHOULDNT BE {x_dimen=} {brick_type_value} : {get_default_sorted_list(idea_attrs)}"
                    )
                    # print(
    ...
    for brick_type in brick_types_sorted:
        ...
        for x_dimen in dimens_list:
            if expected_idea_dimen_ref.get(x_dimen) is None:
                expected_idea_dimen_ref[x_dimen] = {brick_type}
            else:
                expected_idea_dimen_ref.get(x_dimen).add(brick_type)
