...
def test_get_idea_into_dimen_raw_query_ReturnsObj_Scenario0_person_plan_laborunit(
    ...
    gen_sqlstr = get_idea_into_dimen_raw_query(
        cursor0, idea_type, prnlabo_cat, prnlabo_jkeys
    )

    # THEN
    columns_str = f"{kw.spark_num}, {kw.spark_face}, {kw.person_name}, {kw.plan_rope}, {kw.labor_title}"
    expected_sqlstr = f"""INSERT INTO {prnlabo_cat}_raw ({kw.idea_type}, {columns_str})
SELECT '{idea_type}' as {kw.idea_type}, {columns_str}
FROM {idea_type}_raw
...
def test_get_idea_into_dimen_raw_query_ReturnsObj_Scenario1_person_contactunit(
    ...
    # WHEN
    gen_sqlstr = get_idea_into_dimen_raw_query(
        cursor0, idea_type, kw.person_contactunit, prncont_jkeys
    )

    # THEN
    columns_str = "spark_num, spark_face, moment_rope, person_name, contact_name, contact_cred_lumen, contact_debt_lumen"
    expected_sqlstr = f"""INSERT INTO {kw.person_contactunit}_raw (idea_type, {columns_str})
SELECT '{idea_type}' as idea_type, {columns_str}
FROM {idea_type}_raw
