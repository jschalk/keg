...
def get_validated_b_src_brick_type_sheets(
    b_src_dir: str, k_src_dir: str
) -> List[Tuple[str, str]]:
    """
    Returns all brick_type sheets found in b_src_dir.
    Raises a ValueError if any of those brick_type sheets also exist in k_src_dir.

    Args:
        b_src_dir: Path to the BELE source directory.
        k_src_dir: Path to the BRICK source directory.
    ...
    Raises:
        ValueError: If any brick_type sheet found in b_src_dir also exists
                    in k_src_dir (matched on sheet_name alone).
    """
    bele_bk_sheets = get_sheets_with_brick_types(b_src_dir)
    brick_bk_sheets = get_sheets_with_brick_types(k_src_dir)
    ...
    if overlapping := brick_bk_sheets_set.intersection(bele_bk_sheets_set):
        exception_str = "brick_type sheets found in both b_src_dir and k_src_dir: "
        raise ValueError(exception_str, f"{sorted(overlapping)}")

    return bele_bk_sheets


def beliefs_sheets_to_brick_sheets(
    b_src_dir: str, k_src_dir: str, db_max_spark_num: int = None
) -> List[Tuple[str, str]]:
    """
    Copies all brick_type sheets from b_src_dir into k_src_dir.
    ...
    brick_max_spark_num = get_0_if_None(get_max_spark_num_from_files(k_src_dir))
    ...
    bele_bk_sheets = get_validated_b_src_brick_type_sheets(b_src_dir, k_src_dir)
    ...
    for filename, sheet_names in file_to_sheets.items():
        ...
        dst_path = os_path_join(k_src_dir, filename)
