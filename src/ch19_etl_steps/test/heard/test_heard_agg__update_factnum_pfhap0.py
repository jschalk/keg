...
def pfhap0_insert_prnfact(cursor0: Cursor, x_values: list[list]) -> str:
...
def test_get_update_prnfact_inx_epoch_diff_sqlstr_SQLTEST_One_row(
    cursor0: Cursor,
):
    # ESTABLISH
    spark7 = 7
    ...
    print(prnfact_insert_sql)

    # BEFORE
    assert pfhap0_select_prnfact(cursor0) == [(7, wx.clean_rope, None)]
...
def test_get_update_prnfact_inx_epoch_diff_sqlstr_SQLTEST_Two_rows(
    cursor0: Cursor,
):
    # ESTABLISH
    spark7, spark9 = (7, 9)
    ...
    pfhap0_insert_nabtime(cursor0, nabtime_vals)
    prnfact_vals = [[spark7, wx.clean_rope], [spark9, wx.clean_rope]]
    pfhap0_insert_prnfact(cursor0, prnfact_vals)

    # BEFORE
    assert pfhap0_select_prnfact(cursor0) == [
...
def test_get_update_prnfact_inx_epoch_diff_sqlstr_SQLTEST_Three_rows_Two_person_name(
    cursor0: Cursor,
):
    # ESTABLISH
    spark7, spark9 = (7, 9)
