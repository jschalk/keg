...
"""
    ...
    # THEN
    assert db_table_exists(cursor0, idea_sparks_tablename)
    idea_sparks_table_cols = set(get_table_columns(cursor0, idea_sparks_tablename))
    assert len(idea_sparks_table_cols) == 4
    assert kw.idea_type in idea_sparks_table_cols
    assert kw.spark_face in idea_sparks_table_cols
    assert kw.spark_num in idea_sparks_table_cols
    assert kw.error_message in idea_sparks_table_cols
    assert get_row_count(cursor0, idea_sparks_tablename) == 3
    select_agg_sqlstr = f"""
SELECT * 
...
ORDER BY {kw.spark_num}, {kw.spark_face};"""
    ...
    assert len(rows) == 3
    sue_r = ("ii00103", spark1, exx.sue, None)
    yao3_r = ("ii00103", spark3, exx.yao, None)
    yao9_r = ("ii00103", spark9, exx.yao, None)
    print(f"{rows[0]=}")
    assert rows[0] == sue_r
...
def test_etl_ideax_agg_tables_to_sparks_ideax_agg_table_PopulatesTables_Scenario1(
    cursor0: Cursor,
):
    # ESTABLISH
    spark1 = 1
...
"""
    ...
    cursor0.execute(insert_sqlstr)
    idea_sparks_tablename = kw.sparks_ideax_agg
    assert get_row_count(cursor0, agg_ii00103_tablename) == 5
    assert not db_table_exists(cursor0, idea_sparks_tablename)
...
"""
    ...
    # WHEN
    sparks_dict = etl_sparks_ideax_agg_db_to_spark_dict(cursor0)
