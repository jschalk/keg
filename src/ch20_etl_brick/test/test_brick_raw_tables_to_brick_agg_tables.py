...
from ch20_etl_brick.etl_brick_main import etl_brixk_raw_tables_to_brixk_agg_tables
from ref.keywords import Ch20Keywords as kw, ExampleStrs as exx
from sqlite3 import Cursor


def test_etl_brixk_raw_tables_to_brixk_agg_tables_PopulatesAggTable_Scenario0_GroupByWorks_bk00103(
    cursor0: Cursor,
):
    # ESTABLISH
    spark1 = 1
    ...
    create_brick_sorted_table(cursor0, raw_bk00103_tablename, raw_bk00103_columns)
    insert_into_clause = f"""INSERT INTO {raw_bk00103_tablename} (
  {kw.spark_num}
, {kw.spark_face}
, {kw.moment_rope}
, {kw.cumulative_minute}
...
"""
    ...
    # THEN
    assert db_table_exists(cursor0, agg_bk00103_tablename)
    assert get_row_count(cursor0, agg_bk00103_tablename) == 2

    bk00103_table_cols = get_table_columns(cursor0, agg_bk00103_tablename)
    file_dir_str = "file_dir"
    filename_str = "filename"
    sheet_name_str = "sheet_name"
    assert file_dir_str not in set(bk00103_table_cols[0])
    assert filename_str not in set(bk00103_table_cols[1])
    assert sheet_name_str not in set(bk00103_table_cols[2])
    select_agg_sqlstr = f"""
SELECT * 
FROM {agg_bk00103_tablename} 
...
def test_etl_brixk_raw_tables_to_brixk_agg_tables_PopulatesAggTable_Scenario1_GroupByOnlyNonConflictingRecords(
    cursor0: Cursor,
):
    # ESTABLISH
    spark1 = 1
...
"""
    ...
    # THEN
    assert db_table_exists(cursor0, agg_bk00103_tablename)
    assert get_row_count(cursor0, agg_bk00103_tablename) == 1
    ...
    assert filename_str not in set(bk00103_table_cols[1])
    assert sheet_name_str not in set(bk00103_table_cols[2])
    select_agg_sqlstr = f"""SELECT * FROM {agg_bk00103_tablename};"""
    cursor0.execute(select_agg_sqlstr)
...
def test_etl_brixk_raw_tables_to_brixk_agg_tables_PopulatesAggTable_Scenario2_GroupByExcludesRowsWith_error_message(
    cursor0: Cursor,
):
    # ESTABLISH
    spark1 = 1
...
def test_etl_brixk_raw_tables_to_brixk_agg_tables_PopulatesAggTable_Scenario3_TableDeleteBeforeLoad(
    cursor0: Cursor,
):
    # ESTABLISH
    spark1 = 1
    ...
    create_brick_sorted_table(cursor0, raw_bk00105_tablename, raw_bk00105_columns)
    insert_into_clause = f"""INSERT INTO {raw_bk00105_tablename} (
  {kw.spark_num}
, {kw.spark_face}
, {kw.moment_rope}
, {kw.weekday_order}
...
"""
    ...
    etl_brixk_raw_tables_to_brixk_agg_tables(cursor0)
    assert get_row_count(cursor0, agg_bk00105_tablename) == 3
    values2_clause = f"""
VALUES     
  ('{spark1}', '{exx.sue}', '{exx.a23}', 0, '{exx.Wednesday}', ';', NULL)
, ('{spark1}', '{exx.sue}', '{exx.a23}', 0, '{exx.Wednesday}', ';', NULL)
