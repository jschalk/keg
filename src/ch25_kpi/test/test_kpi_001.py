...
from ch23_lynx.lynx import CREATE_MOMENT_CONTACT_NETS_SQLSTR
...
"""
    ...
    # THEN
    assert get_table_columns(cursor0, moment_kpi001_contact_nets_tablename) == [
        kw.moment_rope,
        kw.person_name,
        kw.net_funds,
        kw.fund_rank,
    ...
    assert get_row_count(cursor0, moment_kpi001_contact_nets_tablename)
    select_sqlstr = f"""
SELECT 
  {kw.moment_rope}
, {kw.person_name}
...
, {kw.fund_rank}
, {kw.pledges_count}
FROM {moment_kpi001_contact_nets_tablename}
"""
...
"""
    ...
    # THEN
    assert get_row_count(cursor0, moment_kpi001_contact_nets_tablename)
    select_sqlstr = f"""SELECT {kw.moment_rope}, {kw.person_name}, {kw.net_funds}, {kw.fund_rank}, {kw.pledges_count} FROM {moment_kpi001_contact_nets_tablename}"""
    cursor0.execute(select_sqlstr)
    rows = cursor0.fetchall()
    print(rows)
    assert rows == [
        (exx.a23, exx.bob, bob_contact_net, 1, 1),
        (exx.a23, exx.yao, yao_contact_net, 2, 0),
    ]
