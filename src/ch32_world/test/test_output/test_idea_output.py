...
def test_create_ideas_CreatesFile_Senario0_EmptyWorld(temp3_fs):
    ...
    fay_lego0001_path = create_lego0001_path(fay_wdir.output_dir)
    assert os_path_exists(fay_lego0001_path) is False
...
def test_create_ideas_CreatesFile_Senario2_CreatedIdeaCanBeBricksForOtherWorldDir(
    ...
    shutil_copy2(fay_lego0001_path, dst=bob_b_src_dir_st0001_path)
    # print(f" {pandas_read_excel(fay_lego0001_path)=}")
    ...
    brick_sheets_to_lego_mstr(fay_wdir)
    bob_lego0001_path = create_lego0001_path(bob_wdir.output_dir)
    assert os_path_exists(bob_lego0001_path) is False
    ...
    # THEN
    assert os_path_exists(bob_lego0001_path)
    print(f"{get_sheet_names(bob_lego0001_path)=}")
    for sheetname in get_sheet_names(bob_lego0001_path):
        print(f"comparing {sheetname=}...")
        fay_sheet_df = pandas_read_excel(fay_lego0001_path, sheetname)
        bob_sheet_df = pandas_read_excel(fay_lego0001_path, sheetname)
...
# def test_WorldDir_sheets_b_src_dir_to_lego_CreatesFiles(temp3_fs):
#     # ESTABLISH
#     fay_str = "Fay"
